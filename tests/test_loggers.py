...
def _stderr_text(capsys) -> str:
    # neurodags routes structlog output to stderr (see #15).
...
def test_json_render_when_not_tty(monkeypatch, capsys):
    # Don't replace stderr; just make it report "not a TTY"
...
def test_level_filtering(capsys, monkeypatch):
    monkeypatch.setattr(sys.stderr, "isatty", lambda: False, raising=False)  # JSON easier to check
...
def test_bound_context_present(capsys, monkeypatch):
    monkeypatch.setattr(sys.stderr, "isatty", lambda: False, raising=False)  # JSON
    configure_logging(level="INFO")
    get_logger("ctx", user="alice", session="s1").info("evt", x=1)

    data = _last_json_line(_stderr_text(capsys))
