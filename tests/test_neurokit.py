...
def _make_dataarray() -> xr.DataArray:
    data = np.vstack(
        [
            nk.signal_simulate(
                duration=2.0, sampling_rate=100, frequency=5.0, noise=0.01, random_state=0
            ),
            nk.signal_simulate(
                duration=2.0, sampling_rate=100, frequency=8.0, noise=0.01, random_state=1
            ),
        ]
    )
    return xr.DataArray(
        data,
        dims=("spaces", "times"),
        coords={"spaces": ["A", "B"], "times": np.arange(data.shape[1])},
    )
...
def test_complexity_k_node_produces_dataset_with_value_and_metadata() -> None:
    ...
    value_da = dataset["value"]
    expected = np.array(
        [nk.complexity_k(signal, k_max="max", show=False)[0] for signal in da.values]
    )
    np.testing.assert_allclose(value_da.values, expected, rtol=1e-6, atol=1e-6)
