...
def fooof_component(
    ...
    *,
    ...
    component : {"aperiodic", "periodic", "residual"}, optional
        ...
        the modelled oscillatory power (Gaussians minus background), and
        ``residual`` subtracts the background from the original power spectrum.
    freq_dim : str, optional
    ...
    component_data = np.full((flat_count, freq_len), np.nan, dtype=float)
    ...
    for idx, fm in enumerate(loaded_models):
        ...
        try:
            ...
            # according to https://fooof-tools.github.io/fooof/auto_tutorials/plot_03-FOOOFAlgorithm.html
            # we substract from power_spectrum, not from the model fit.
            periodic_linear = power_log - ap_fit_log
            # periodic_linear = np.clip(model_linear - ap_linear, a_min=0.0, a_max=None)
            # not sure about the clip

            if component == "periodic":
                component_data[idx] = periodic_linear
                continue

            residual_linear = np.power(10.0, power_log) - model_linear
            component_data[idx] = residual_linear
        except Exception as exc:  # noqa: BLE001
            log.warning("Failed to compute FOOOF component", index=idx, error=str(exc))
            invalid_indices.append(idx)
            component_data[idx] = np.nan

    output_shape = tuple(other_shape + [freq_len]) if other_dims else (freq_len,)
    reshaped = component_data.reshape(output_shape)
    ...
    component_xr = xr.DataArray(reshaped, dims=output_dims, coords=coords, name=name)
    ...
    if fooof_meta:
        component_metadata["fooof_metadata"] = fooof_meta
