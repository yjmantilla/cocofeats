...
def basic_preprocessing(mne_object, downsample=None, filter_args=None, epoch_config=None, notch_filter=None, extra_artifacts: bool = False) -> FeatureResult:
    ...
    report = mne.Report(title=f'Basic Preprocessing', verbose='error')

    # EEG before preprocessing:
    if extra_artifacts:
        report.add_figure(mne_object.plot(show=False), title=f'Raw Data Before Preprocessing')
        log.debug("MNEReport: added raw data figure")
        # psd
        report.add_figure(mne_object.compute_psd().plot(show=False), title=f'Power Spectral Density Before Preprocessing')
    ...
    if epoch_config is not None:
        ...
        if extra_artifacts:
            report.add_figure(mne_object.plot(show=False), title=f'Epochs After Segmentation')
    ...
    if extra_artifacts:
        artifacts['.report.html'] = Artifact(item=report, writer=lambda path: report.save(path, overwrite=True, open_browser=False))
