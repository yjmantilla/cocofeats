...
def register_flows_from_dict(yaml_definitions: dict):
    for name, chain in yaml_definitions.get("FeatureDefinitions", {}).items():
        ...
        register_flow_with_name(name, func, definition=chain, override=True)
        log.info("Registered flow", name=name)
register_flows_from_dict(yaml_definitions)